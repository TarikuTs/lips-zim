...
class CheeseburgerMenuService {
   ...
   */
  public function __construct(EntityTypeManagerInterface $entity_type_manager, MenuLinkTreeInterface $menu_link_tree, MenuActiveTrailInterface $menu_active_trail, CurrentRouteMatch $current_route_match) {
  ...
  public function buildMenu(array $menu_settings, $parent_menu_as_link = FALSE) {
    ...
    if (!is_null($max_depth) && is_numeric($max_depth) && $max_depth > 0) {
      $menu_tree_parameters->setMaxDepth($max_depth);
    }
  ...
  public function buildMenuFromVocabulary(array $vocabulary_menu_settings, $parent_menu_as_link = FALSE) {
    ...
    if ($route_parameters->has('taxonomy_term')) {
      ...
      $parents = $term_storage->loadAllParents(is_numeric($current_taxonomy_term) ? $current_taxonomy_term : $current_taxonomy_term->id());
    ...
    foreach ($top_tree_taxonomy_terms as $taxonomy_term) {
      if ($taxonomy_term->access('view')) {
        if ($vocabulary_menu_settings['settings']['show_links_in_navigation']) {
          $cheeseburger_menu->addNavigationMenuItem(CheeseburgerMenuItem::createFromTaxonomyVocabulary($taxonomy_term, $menu_item_settings, $max_depth));
        }
        else {
          $cheeseburger_menu->addMenuItem(CheeseburgerMenuItem::createFromTaxonomyVocabulary($taxonomy_term, $menu_item_settings, $max_depth));
        }
      }
    }
    return $cheeseburger_menu;
