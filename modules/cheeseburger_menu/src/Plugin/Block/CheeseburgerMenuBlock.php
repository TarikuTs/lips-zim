...
class CheeseburgerMenuBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function submitConfigurationForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->setConfigurationValue('left_side_background_color', $form_state->getValue('left_side_background_color'));
    $this->setConfigurationValue('left_side_text_color', $form_state->getValue('left_side_text_color'));
    $this->setConfigurationValue('right_side_background_color', $form_state->getValue('right_side_background_color'));
    $this->setConfigurationValue('right_side_text_color', $form_state->getValue('right_side_text_color'));
    $this->setConfigurationValue('trigger_color', $form_state->getValue('trigger_color'));
    $this->setConfigurationValue('trigger_background_color', $form_state->getValue('trigger_background_color'));
    $this->setConfigurationValue('scrollbar_color', $form_state->getValue('scrollbar_color'));
    foreach ([
      'left_side_background_opacity',
      'left_side_text_opacity',
      'right_side_background_opacity',
      ...
      $this->setConfigurationValue($opacity_setting, $form_state->getValue($opacity_setting));
    }
    // Config group.
    $menus = [];
    ...
    foreach ($this->getConfigValue('menus') as $menu) {
      switch ($menu['menu_type']) {
        case 'taxonomy_vocabulary':
          $menus[$menu['id']] = $this->service->buildMenuFromVocabulary($menu, $this->getConfigValue('parent_menu_as_link'));
          break;

        case 'menu':
          $menus[$menu['id']] = $this->service->buildMenu($menu, $this->getConfigValue('parent_menu_as_link'));
          break;
      }
    }
  ...
  public function getCacheTags() {
    ...
    foreach ($this->configuration['menus'] as $menu) {
      ...
      elseif ($menu['menu_type'] === 'taxonomy_vocabulary') {
        $cache_tags[] = 'taxonomy_vocabulary:' . $menu['id'];
      }
    }
