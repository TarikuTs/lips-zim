(function (Drupal, $, _) {
  Drupal.behaviors.cheeseburgermenuMain = {
    attach: function (context) {
      ...
      $(MENU_TRIGGER, context).each(function () {
      ...
      function handleTriggerClick(e) {
        ...
        var id = $(e.target).attr(TRIGGER) || $(e.target).parent().attr(TRIGGER);
        if ($(`#${id}`).attr("style")) {
          $(`#${id}`).removeAttr("style");
          setTimeout(function () {
      ...
      function handleCloseClick(e) {
        ...
        e.preventDefault();
        e.stopPropagation();
        $(CHEESEBURGER_ACTIVE).eq(0).removeClass(CHEESEBURGER_ACTIVE.replace(".", ""));
      ...
      function handleParentTriggerClick(e) {
        ...
        if ($parentItem.hasClass(CHEESEBURGER_MENU_ITEM_ACTIVE.replace(".", ""))) {
          // Toggle already opened submenu.
          $.each($parentItem.children("ul"), function (i) {
            animateToHeightAndClearStyle($(this), 0, $parentItem.children("ul").length);
          });

          setTimeout(function () {
            $parentItem.removeClass(CHEESEBURGER_MENU_ITEM_ACTIVE.replace(".", ""));
          }, ANIMATION_SPEED * 0.8);
        } else {
          ...
          $.each(
            ...
            function () {
              $.each($(this).children("ul"), function () {
                animateToHeightAndClearStyle($(this), 0, $(this).children("ul").length);
              });
            }
          );

          $.each($parentItem.siblings(CHEESEBURGER_MENU_ITEM).not(DEFAULT_EXPAND), function () {
            $.each($(this).children("ul"), function () {
              animateToHeightAndClearStyle($(this), 0, $(this).children("ul").length);
            });

            $(this).removeClass(CHEESEBURGER_MENU_ITEM_ACTIVE.replace(".", ""));
          });

          $parentItem.parents("ul").siblings("ul").find(`> ${CHEESEBURGER_MENU_ITEM}`).removeClass(CHEESEBURGER_MENU_ITEM_ACTIVE.replace(".", ""));
          ...
          $.each($parentItem.children("ul"), function () {
            ...
            $(this).css({ height: 0 });
            animateToHeightAndClearStyle($(this), height, $parentItem.children("ul").length);
          });
        }
      }
      ...
      function animateToHeightAndClearStyle($el, height, coeficient) {
        ...
        $el.animate({ height: height }, ANIMATION_SPEED * variation, function () {
      ...
      function handleMiscClick(e) {
        if ($(e.target).parents(CHEESEBURGER_ACTIVE).length === 0) {
          $(CHEESEBURGER_ACTIVE).each(function () {
            ...
            $(this).find(CHEESE_HIGHLIGHTED).removeClass(CHEESE_HIGHLIGHTED.replace(".", ""));
            $(BACKDROP).removeClass(BACKDROP_ACTIVE.replace(".", ""));
            $('body').removeClass(BODY_ACTIVE.replace(".", ""));
          });
        }
      ...
      function handleSideTriggerClick(e) {
        ...
        var $targetId = $(e.target).attr(TRIGGER) || $(e.target).parents(SIDE_TRIGGER).attr(TRIGGER);
        var $target = $(e.target).parents(CHEESEBURGER).find(CHEESEBURGER_MAINMENU).find(`[${TRIGGER}=${$targetId}]`);
        $target.addClass(CHEESE_HIGHLIGHTED.replace(".", ""));
        var $offsetTopFromMainArea = $target.position().top;
        var $currentlyScrolled = $(e.target).parents(CHEESEBURGER).find(CHEESEBURGER_MAINMENU).scrollTop();

        $(e.target)
          .parents(CHEESEBURGER)
          .eq(0)
          .find(CHEESEBURGER_MAINMENU)
          .animate({ scrollTop: $offsetTopFromMainArea + $currentlyScrolled }, 300);
      }
