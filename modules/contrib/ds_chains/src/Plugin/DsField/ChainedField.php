...
class ChainedField extends DsFieldBase {
  ...
  public function build() {
    ...
    foreach ($entity->get($field_name) as $delta => $field_item) {
      ...
      if (!($chained_entity = $field_item->get('entity')->getValue())
        || !$chained_entity->hasField($chained_field)
        || $chained_entity->get($chained_field)->isEmpty()
        || !$chained_entity->get($chained_field)->access('view')) {
        // Entity/field doesn't exist or chained field is empty.
        if ($chained_entity) {
